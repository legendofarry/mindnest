...
void FirebaseAuthPlugin::RegisterWithRegistrar(
  ...
  auto plugin = std::make_unique<FirebaseAuthPlugin>(registrar);
...
FirebaseAuthPlugin::FirebaseAuthPlugin(
  ...
  dispatch_message_id_ =
      RegisterWindowMessageW(L"firebase_auth_windows_dispatch");

  if (auto* view = registrar_->GetView()) {
    HWND view_window = view->GetNativeWindow();
    window_handle_ = GetAncestor(view_window, GA_ROOT);
    if (window_handle_ == nullptr) {
      window_handle_ = view_window;
    }
  }
...
void FirebaseAuthPlugin::PostToPlatformThread(std::function<void()> task) {
  {
    std::lock_guard<std::mutex> lock(pending_tasks_mutex_);
    pending_tasks_.push(std::move(task));
  }

  if (window_handle_ != nullptr && dispatch_message_id_ != 0) {
    PostMessage(window_handle_, dispatch_message_id_, 0, 0);
  }
}

std::optional<LRESULT> FirebaseAuthPlugin::HandleTopLevelWindowProc(
    HWND hwnd,
    UINT message,
    WPARAM wparam,
    LPARAM lparam) {
  if (message != dispatch_message_id_ || hwnd != window_handle_) {
    return std::nullopt;
  }

  DrainPendingPlatformTasks();
  return 0;
}

void FirebaseAuthPlugin::DrainPendingPlatformTasks() {
...
struct PlatformThreadEventSinkState {
  std::mutex mutex;
...
void PostResultToPlatformThread(FirebaseAuthPlugin* plugin,
                                Callback callback,
                                Arg arg) {
  plugin->PostToPlatformThread(
      [callback = std::move(callback), arg = std::move(arg)]() mutable {
...
class FlutterIdTokenListener : public firebase::auth::IdTokenListener {
 public:
  explicit FlutterIdTokenListener(FirebaseAuthPlugin* plugin)
      : plugin_(plugin),
        sink_state_(std::make_shared<PlatformThreadEventSinkState>()) {}

  void SetEventSink(
      std::unique_ptr<flutter::EventSink<flutter::EncodableValue>> event_sink) {
    std::lock_guard<std::mutex> lock(sink_state_->mutex);
    sink_state_->generation++;
    sink_state_->event_sink = event_sink
                                  ? std::shared_ptr<
                                        flutter::EventSink<
                                            flutter::EncodableValue>>(
                                        std::move(event_sink))
  ...
  void OnIdTokenChanged(Auth* auth) override {
    ...
    {
      std::lock_guard<std::mutex> lock(sink_state_->mutex);
      event_sink = sink_state_->event_sink;
      generation = sink_state_->generation;
    }
    ...
    EncodableValue payload = user.is_valid()
                                 ? EncodableValue(EncodableMap{{
                                       EncodableValue("user"),
                                       EncodableList{
                                           userDetails.user_info()
                                               .ToEncodableList(),
                                           userDetails.provider_data(),
                                       },
                                   }})
                                 : EncodableValue(EncodableMap{{
                                       EncodableValue("user"),
    ...
    plugin_->PostToPlatformThread(
        [sink_state = sink_state_, event_sink = std::move(event_sink),
         generation, payload = std::move(payload)]() mutable {
          {
            std::lock_guard<std::mutex> lock(sink_state->mutex);
            if (!sink_state->event_sink ||
                sink_state->generation != generation) {
              return;
            }
          }
          event_sink->Success(payload);
        });
  }

 private:
  FirebaseAuthPlugin* plugin_;
  std::shared_ptr<PlatformThreadEventSinkState> sink_state_;
};

class IdTokenStreamHandler
    : public flutter::StreamHandler<flutter::EncodableValue> {
 public:
  IdTokenStreamHandler(Auth* auth, FirebaseAuthPlugin* plugin) {
    listener_ = nullptr;
    auth_ = auth;
    plugin_ = plugin;
  }
  ...
  OnListenInternal(
      ...
      override {
    listener_ = new FlutterIdTokenListener(plugin_);
...
void FirebaseAuthPlugin::RegisterIdTokenListener(
  ...
  auto id_token_handler =
      std::make_unique<IdTokenStreamHandler>(firebaseAuth, this);
...
class FlutterAuthStateListener : public firebase::auth::AuthStateListener {
 public:
  explicit FlutterAuthStateListener(FirebaseAuthPlugin* plugin)
      : plugin_(plugin),
        sink_state_(std::make_shared<PlatformThreadEventSinkState>()) {}
...
class AuthStateStreamHandler
    : public flutter::StreamHandler<flutter::EncodableValue> {
 public:
  AuthStateStreamHandler(Auth* auth, FirebaseAuthPlugin* plugin) {
    listener_ = nullptr;
    auth_ = auth;
    plugin_ = plugin;
  }
  ...
  OnListenInternal(
      ...
      std::unique_ptr<flutter::EventSink<flutter::EncodableValue>>&& events)
      override {
    listener_ = new FlutterAuthStateListener(plugin_);
    listener_->SetEventSink(std::move(events));
...
void FirebaseAuthPlugin::RegisterAuthStateListener(
  ...
  auto auth_state_handler =
      std::make_unique<AuthStateStreamHandler>(firebaseAuth, this);
...
void FirebaseAuthPlugin::CreateUserWithEmailAndPassword(
  ...
  createUserFuture.OnCompletion(
      [plugin = this, result](const firebase::Future<firebase::auth::AuthResult>&
                   completed_future) {
        if (completed_future.error() == 0) {
          PigeonUserCredential credential =
              ParseAuthResult(completed_future.result());
          PostResultToPlatformThread(
              plugin, result, ErrorOr<PigeonUserCredential>(credential));
        } else {
          PostResultToPlatformThread(
              plugin, result,
              ErrorOr<PigeonUserCredential>(
                  FirebaseAuthPlugin::ParseError(completed_future)));
        }
      });
}
  ...
  signInFuture.OnCompletion(
      [plugin = this, result](
        ...
        if (completed_future.error() == 0) {
          ...
          PostResultToPlatformThread(
              plugin, result,
              ErrorOr<PigeonUserCredential>(userCredential));
        } else {
          PostResultToPlatformThread(
              plugin, result,
              ErrorOr<PigeonUserCredential>(
        ...
        if (completed_future.error() == 0) {
          PostResultToPlatformThread(
              ...
              ErrorOr<flutter::EncodableList>(
                  TransformStringList(completed_future.result()->providers)));
        } else {
          PostResultToPlatformThread(
              plugin, result,
              ErrorOr<flutter::EncodableList>(
...
void FirebaseAuthPlugin::SendPasswordResetEmail(
  ...
  signInFuture.OnCompletion(
      [plugin = this, result](const firebase::Future<void>& completed_future) {
        if (completed_future.error() == 0) {
          PostResultToPlatformThread(
              plugin, result, std::optional<FlutterError>(std::nullopt));
        } else {
          PostResultToPlatformThread(
              plugin, result,
  ...
  future.OnCompletion(
      [plugin = this, result](
        ...
        if (completed_future.error() == 0) {
          ...
          PostResultToPlatformThread(
              plugin, result, ErrorOr<PigeonIdTokenResult>(token_result));
        } else {
          PostResultToPlatformThread(
              plugin, result,
              ErrorOr<PigeonIdTokenResult>(
                  FirebaseAuthPlugin::ParseError(completed_future)));
        }
      });
}
...
void FirebaseAuthPlugin::Reload(
  ...
  future.OnCompletion([plugin = this, result, firebaseAuth](
                          const firebase::Future<void>& completed_future) {
    if (completed_future.error() == 0) {
      PigeonUserDetails user = ParseUserDetails(firebaseAuth->current_user());
      PostResultToPlatformThread(
          plugin, result, ErrorOr<PigeonUserDetails>(user));
    } else {
      PostResultToPlatformThread(
          plugin, result,
          ErrorOr<PigeonUserDetails>(
              FirebaseAuthPlugin::ParseError(completed_future)));
    }
  });
}
